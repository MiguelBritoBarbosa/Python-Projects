...
print(Fore.CYAN + "desenvolvido por: Miguel Brito - Lucas Maia - Agnes Bakos" + Fore.RESET)
print(Back.BLACK + Fore.LIGHTWHITE_EX +  "-"*50)
...
def Tables():
    ...
    '''
    grid = PrettyTable(["Id", "Selecione o local que deseja consultar"])
    try:
        query = conexão.cursor()
        query.execute("show tables;")
        tabela = query.fetchall()
        for i in range(len(tabela)):
            tabela[i] = (i, tabela[i][0])
        if query.rowcount > 0:
...
def UpdateTable(NomeTabela, Id):
    ...
    try:
        ...
        if NomeTabela != "disciplinasxprofessores_tb":
            ...
            if NomeTabela == "disciplinas_tb":
                if ChaveEstrangeira[0][1] == int(Id):
                    print(f"Essa disciplina está vinculada a um professor! Desfaça o relacionamento para prosseguir com a alteração!")
                    resp = input("Deseja desvincular a disciplina?(s/n)")
                    ValidarResposta(resp)
                    if resp == "s":
                        query.execute(f"delete from disciplinasxprofessores_tb where codigoDisciplina = {Id}")
                        print("Relacionamento desfeito com sucesso!")
                        resp = input("Prosseguir com a alteração?(s/n)\n")
                        ValidarResposta(resp)
                        if resp == "s":
                            query.execute(f"select * from {NomeTabela} where {NomeColunas[0]} = {Id};")
                            tabela = query.fetchall()
                ...
                if ChaveEstrangeira[0][2] == int(Id):
                    print(
                        f"Esse professor está vinculado a uma disciplina! Desfaça o relacionamento para prosseguir com a alteração")
                    resp = input("Deseja desvincular a disciplina?(s/n)")
                    ValidarResposta(resp)
                    if resp == "s":
                        query.execute(f"delete from disciplinasxprofessores_tb where codigoProfessor = {Id}")
                        print("Relacionamento desfeito com sucesso!")
                        resp = input("Prosseguir com a alteração?(s/n)\n")
                        ValidarResposta(resp)
                        if resp == "s":
...
def Delete(NomeTabela, Id):
    try:
        ...
        if NomeTabela != "disciplinasxprofessores_tb":
            ...
            if NomeTabela == "disciplinas_tb":
                if ChaveEstrangeira[0][1] == int(Id):
                    print(f"Essa disciplina está vinculada a um professor! Desfaça o relacionamento para prosseguir com a exclusão!")
                    resp = input("Deseja desvincular a disciplina?(s/n)")
                    ValidarResposta(resp)
                    if resp == "s":
                        query.execute(f"delete from disciplinasxprofessores_tb where codigoDisciplina = {Id}")
                        print("Relacionamento desfeito com sucesso!")
                        resp = input("Prosseguir com a exclusão?(s/n)\n")
                        ValidarResposta(resp)
                        if resp == "s":
                ...
                if ChaveEstrangeira[0][2] == int(Id):
                    print(f"Esse professor está vinculado a uma disciplina! Desfaça o relacionamento para prosseguir com a exclusão")
                    resp = input("Deseja desvincular a disciplina?(s/n)")
                    ValidarResposta(resp)
                    if resp == "s":
                        query.execute(f"delete from disciplinasxprofessores_tb where codigoProfessor = {Id}")
                        ...
                        query.close()
                        return 0
        query.execute(f"select * from {NomeTabela} where {NomeColunas[0]} = {int(Id)}")
        tabela = query.fetchall()
        if len(tabela) > 0:
            query.execute(f"delete from {NomeTabela} where {NomeColunas[0]} = {int(Id)};")
            conexão.commit()
            ...
            print("Esse registro não existe no contexto atual!")
            return 0
